...
---
Vered: To open a saved R session use: load("something.R")
...
source ("30.Functions_for_DESeq2_analysis.R")
...
#analyst_name = "Vered_Caspi"
#final_results_file          = sprintf("%s/RESULTS_%s_%s_%s.xlsx", results_dir,analysis_name, analyst_name, current_date)
#final_results_ALL_file      = sprintf("%s/RESULTS_ALL_%s_%s_%s.xlsx", results_dir,analysis_name, analyst_name, current_date)
#final_results_DE_file       = sprintf("%s/RESULTS_DE_%s_%s_%s.xlsx", results_dir,analysis_name, analyst_name, current_date)
...
```
# External Annotation 
(from external file, e.g. from Ensmbl)
...
#Vered added on 24.9.2020 due to error " Error in DESeqDataSetFromTximport(txi = txi.rsem, colData = col_data,  : all(lengths > 0) is not TRUE
#(see https://support.bioconductor.org/p/92763/)
...
#this function works (for volcano), but the PDF file it creates is too big and makes problems after opening it.
#draw_ma_and_volcano_plots_pdf (dds, contrasts_file, results_dir)
...
#Alternatively, get gene names from file, assuming they are in the first column
...
```
# Correlation to binary patterns
...
## Alternatively, find correlation to one given pattern
...
## Heatmap for DE genes any contrast
...
```
## Heatmap for DE genes combined (any contrast or interaction)

```{r hierarchical_clustering}

#plot heatmap for a list of genes (in this case, top combined genes, i.e. DE any contrast or significant interaction)
#add col names from col_data and row names from "up", "down" per contrast and from interaction from stats_df_interaction
...
## Heatmap for all binary patterns in DE genes
...
mat2plot_means_scaled <- mat2plot_means %>% t %>% scale %>% t

#reduce no. of digits after the dot
#mat2plot_means_scaled = apply(mat2plot_means_scaled,    
#                               MARGIN = c(1,2),
...
#plot graphs with smoothing
# Clustering_Plot2 = plot_cluster_profiles(clusters,mat2plot,col_data, c(GROUP1,GROUP),c(GROUP1,GROUP,"Normalized counts"), smooth=T, X_AXIS_ORDER=unique(col_data[GROUP]), out_file = cluster_profiles_file1)
# 
# Clustering_Plot2
...
## Summary stats of current clusters (after z-scoring)
...
#cutoffs are based on z-scores of vsd normalized data
...
## prepare metadata_rows, contrasts_header and grouping_header
...
## Create DE genes, interaction and combined files
...
add_column_description_tab (wb1, grouping_header_DE, contrasts_data, incl_clust_results=TRUE, final_results_DE_file)
...
for (GOType in GO_TYPES){
  ...
  #Vered 26.7.2023 - for delivery to RShiny
  if (GOType == 'BP'){
  ...
  # the code below should be adopted to this script
  
  # #compute enrichment for up- and down- genes (Vered: need to define them in my script, and to prepare subdirectories for the results - for each contrast)
  # 
  # if ((CALC_CONTRASTS) && ((length(down_reg)>0) || (length(up_reg)>0))){
  ...
  #   }
  #   
  #   allRes <- Clusters_Enrichment_Test(enrichment_results_dir,up_down_clusters,GO2name,GO2gene,paste(GOType,'_GO_Enrichment_of_Up_and_Down_Clusters',SPLIT,sep="_"),"GO",pAdjustMethod=ENRICHMENT_PADJ_METHOD,pvalueCutoff=ENRICHMENT_PVAL_CUTOFF,gene2ko=FALSE,maxCategory = MAX_TERMS_IN_DOTPLOT)
  #   main_plot_list <- c(main_plot_list ,list(allRes[[2]]) )
  #   
  #   #plot overlap heatmaps
  #   heatmaps = plot_shared_genes(allRes[[1]]@compareClusterResult,enrichment_results_dir,paste(GOType,'_GO_Enrichment_of_Up_and_Down_Clusters'))
  #   main_plot_list <- c(main_plot_list ,list(heatmaps))
  #   
  # }
  # 
  # #compute enrhchment for all DE genes
  # #in Liron's script these are per contrast. Here I have to modify the code so it is computed per contrast
  # 
  # One_cluster=clusters
  # One_cluster[]=1
  # allRes <- Clusters_Enrichment_Test(enrichment_results_dir,One_cluster,GO2name,GO2gene,paste(GOType,'_GO_Enrichment_of_All_Significant_Genes',SPLIT,sep="_"),"GO",pAdjustMethod=ENRICHMENT_PADJ_METHOD,pvalueCutoff=ENRICHMENT_PVAL_CUTOFF,gene2ko=FALSE,maxCategory = MAX_TERMS_IN_DOTPLOT)
  # main_plot_list <- c(main_plot_list ,list(allRes[[2]]) )
  # 
  # #create heatmaps of gene2term and term2term overlaps
  # heatmaps = plot_shared_genes(allRes[[1]]@compareClusterResult,enrichment_results_dir,paste(GOType,'_GO_Enrichment_of_All_Significant_Genes'))
  # main_plot_list <- c(main_plot_list ,list(heatmaps))
  
}

```
## Compute KEGG enrichment - Ensembl/uniprot

```{r KEGG enrichmentl Ensembl Uniprot}

#ORGANISM_KEGG_flag

opt_KEGG_Enrichment_of = 'pathway'    #can be 'pathway' or 'ko' 

if (opt_KEGG_Enrichment_of == 'ko'){   #Vered: not our case here
  TERMS_Info= create_gene2kegg(Annotation,F)  # I have not tested the function yet
...
# allRes  <- Clusters_Enrichment_Test(term_dir,clusters,ORGANISM_Pathway2name_,ORGANISM_Pathway2gene_,paste(ORGANISM,'KEGG_Enrichment_Analysis_of_Clusters', sep="_"),"KEGG",pAdjustMethod=ENRICHMENT_PADJ_METHOD,pvalueCutoff=ENRICHMENT_PVAL_CUTOFF,gene2ko=FALSE,maxCategory = MAX_TERMS_IN_DOTPLOT)
# Vered 25.7.2023 removed ORGANISM from result file name, because it was not defined
allRes  <- Clusters_Enrichment_Test(term_dir,clusters,ORGANISM_Pathway2name_,ORGANISM_Pathway2gene_,'_KEGG_Enrichment_Analysis_of_Clusters', "KEGG",pAdjustMethod=ENRICHMENT_PADJ_METHOD,pvalueCutoff=ENRICHMENT_PVAL_CUTOFF,gene2ko=FALSE,maxCategory = MAX_TERMS_IN_DOTPLOT)
main_plot_list <- c(main_plot_list ,list(allRes[[2]]) )
...
#per contrast
allRes_contrasts = Contrasts_Enrichment_Test(term_dir, gene_lists$contrasts, ORGANISM_Pathway2name_, ORGANISM_Pathway2gene_, paste(ORGANISM,'KEGG_Enrichment_Analysis_of_Contrasts', sep="_"),"KEGG", pAdjustMethod=ENRICHMENT_PADJ_METHOD, pvalueCutoff=ENRICHMENT_PVAL_CUTOFF,gene2ko=FALSE,maxCategory = MAX_TERMS_IN_DOTPLOT)
...
#The code below should be adjusted to this script (for up- and down- genes and for up+down genes per contrast)
...
#   }
#   
#   allRes <- Clusters_Enrichment_Test(enrichment_results_dir,up_down_clusters,ORGANISM_Pathway2name_,ORGANISM_Pathway2gene_,paste(ORGANISM,'KEGG_Enrichment_Analysis_of_Up_and_Down_Clusters',SPLIT,sep="_"),"KEGG",pAdjustMethod=ENRICHMENT_PADJ_METHOD,pvalueCutoff=ENRICHMENT_PVAL_CUTOFF,gene2ko=FALSE,maxCategory = MAX_TERMS_IN_DOTPLOT)
#   main_plot_list <- c(main_plot_list ,list(allRes[[2]]) )
#   if (opt$Enriched_terms_overlap){
#     heatmaps = plot_shared_genes(allRes[[1]]@compareClusterResult,enrichment_results_dir,'KEGG_Enrichment_Analysis_of_Up_and_Down_Clusters')
#     main_plot_list <- c(main_plot_list ,list(heatmaps))
#   }
#   
# }
...
# One_cluster=clusters
# One_cluster[]=1
# allRes <- Clusters_Enrichment_Test(enrichment_results_dir,One_cluster,ORGANISM_Pathway2name_,ORGANISM_Pathway2gene_,paste(ORGANISM,'KEGG_Enrichment_Analysis_of_All_Significant_Genes',SPLIT,sep="_"),"KEGG",pAdjustMethod=ENRICHMENT_PADJ_METHOD,pvalueCutoff=ENRICHMENT_PVAL_CUTOFF,gene2ko=FALSE,maxCategory = MAX_TERMS_IN_DOTPLOT)
# main_plot_list <- c(main_plot_list ,list(allRes[[2]]) )
# if (opt$Enriched_terms_overlap){
#   heatmaps = plot_shared_genes(allRes[[1]]@compareClusterResult,enrichment_results_dir,'KEGG_Enrichment_Analysis_of_All_Significant_Genes')
#   main_plot_list <- c(main_plot_list ,list(heatmaps))
# }
# 
# if (((length(down_reg)>0) || (length(up_reg)>0)) && (is.na(opt$LRT))){
#   pathway_table = create_pathway_table(enrichment_results_dir,up_reg,down_reg,Annotation,ORGANISM_Pathway2name,ORGANISM_Pathway2gene,Pathway_show_cutoff,T,Pathway_up_color,Pathway_down_color,F)
# } else{
#   pathway_table = create_pathway_table(enrichment_results_dir,sig.genes,c(),Annotation,ORGANISM_Pathway2name,ORGANISM_Pathway2gene,Pathway_show_cutoff,T,Pathway_up_color,Pathway_down_color,F)
...
nr_total_genes = length(unique(ORGANISM_Pathway2gene_[,2]))  #no. of genes having assigned pathways
res=GSEA (
...
## KEGG enrichment with GSEA, by p-value with direction of change  #Vered Aug2023
...
```
## KEGG enrichment with GSEA, by FC with direction of change  #Vered Aug2023
...
## Compute KEGG enrichment - B

```{r KEGG enrichmentl B}

# #KEGG_flag
# 
# opt_KEGG_Enrichment_of = 'pathway'    #can be 'pathway' or 'ko'
# 
# if (opt_KEGG_Enrichment_of == 'ko'){
...
#   Pathway2gene_ = Pathway2gene
# }
# allRes <- Clusters_Enrichment_Test(enrichment_results_dir,clusters,Pathway2name_,Pathway2gene_,paste('KEGG_Enrichment_Analysis_of_Clusters',SPLIT,sep="_"),"KEGG",pAdjustMethod=ENRICHMENT_PADJ_METHOD,pvalueCutoff=ENRICHMENT_PVAL_CUTOFF,gene2ko=FALSE,maxCategory = MAX_TERMS_IN_DOTPLOT)
# main_plot_list <- c(main_plot_list ,list(allRes[[2]]) )
# if (opt$Enriched_terms_overlap){
#   heatmaps = plot_shared_genes(allRes[[1]]@compareClusterResult,enrichment_results_dir,'KEGG_Enrichment_Analysis_of_Clusters')
#   main_plot_list <- c(main_plot_list ,list(heatmaps))
# }
# 
# if (((length(down_reg)>0) || (length(up_reg)>0)) && (is.na(opt$LRT))){
...
#   }
#   
#   allRes <- Clusters_Enrichment_Test(enrichment_results_dir,up_down_clusters,Pathway2name_,Pathway2gene_,paste('KEGG_Enrichment_Analysis_of_Up_and_Down_Clusters',SPLIT,sep="_"),"KEGG",pAdjustMethod=ENRICHMENT_PADJ_METHOD,pvalueCutoff=ENRICHMENT_PVAL_CUTOFF,gene2ko=FALSE,maxCategory = MAX_TERMS_IN_DOTPLOT)
#   main_plot_list <- c(main_plot_list ,list(allRes[[2]]) )
#   if (opt$Enriched_terms_overlap){
#     heatmaps = plot_shared_genes(allRes[[1]]@compareClusterResult,enrichment_results_dir,'KEGG_Enrichment_Analysis_of_Up_and_Down_Clusters')
#     main_plot_list <- c(main_plot_list ,list(heatmaps))
...
# One_cluster=clusters
# One_cluster[]=1
# allRes <- Clusters_Enrichment_Test(enrichment_results_dir,One_cluster,Pathway2name_,Pathway2gene_,paste('KEGG_Enrichment_Analysis_of_All_Significant_Genes',SPLIT,sep="_"),"KEGG",pAdjustMethod=ENRICHMENT_PADJ_METHOD,pvalueCutoff=ENRICHMENT_PVAL_CUTOFF,gene2ko=FALSE,maxCategory = MAX_TERMS_IN_DOTPLOT)
# main_plot_list <- c(main_plot_list ,list(allRes[[2]]) )
# if (opt$Enriched_terms_overlap){
#   heatmaps = plot_shared_genes(allRes[[1]]@compareClusterResult,enrichment_results_dir,'KEGG_Enrichment_Analysis_of_All_Significant_Genes')
#   main_plot_list <- c(main_plot_list ,list(heatmaps))
...
# 
# if (((length(down_reg)>0) || (length(up_reg)>0)) && (is.na(opt$LRT))){
#   pathway_table = create_pathway_table(enrichment_results_dir,up_reg,down_reg,Annotation,Pathway2name,Pathway2gene,Pathway_show_cutoff,T,Pathway_up_color,Pathway_down_color)
# } else{
#   pathway_table = create_pathway_table(enrichment_results_dir,sig.genes,c(),Annotation,Pathway2name,Pathway2gene,Pathway_show_cutoff,T,Pathway_up_color,Pathway_down_color)
# }
# 

```
## Compute KEGG enrichment - C

```{r KEGG enrichmentl C}
...
# }
# 
# allRes <- Clusters_Enrichment_Test(enrichment_results_dir,clusters,KASS_Pathway2name_,KASS_Pathway2gene_,paste('KEGG_Enrichment_Analysis_of_Clusters',SPLIT,sep="_"),"KEGG",pAdjustMethod=ENRICHMENT_PADJ_METHOD,pvalueCutoff=ENRICHMENT_PVAL_CUTOFF,gene2ko=FALSE,maxCategory = MAX_TERMS_IN_DOTPLOT)
# main_plot_list <- c(main_plot_list ,list(allRes[[2]]) )
# if (opt$Enriched_terms_overlap){
#   heatmaps = plot_shared_genes(allRes[[1]]@compareClusterResult,enrichment_results_dir,'KEGG_Enrichment_Analysis_of_Clusters')
#   main_plot_list <- c(main_plot_list ,list(heatmaps))
...
#   }
#   
#   allRes <- Clusters_Enrichment_Test(enrichment_results_dir,up_down_clusters,KASS_Pathway2name_,KASS_Pathway2gene_,paste('KEGG_Enrichment_Analysis_of_Up_and_Down_Clusters',SPLIT,sep="_"),"KEGG",pAdjustMethod=ENRICHMENT_PADJ_METHOD,pvalueCutoff=ENRICHMENT_PVAL_CUTOFF,gene2ko=FALSE,maxCategory = MAX_TERMS_IN_DOTPLOT)
#   main_plot_list <- c(main_plot_list ,list(allRes[[2]]) )
#   if (opt$Enriched_terms_overlap){
#     heatmaps = plot_shared_genes(allRes[[1]]@compareClusterResult,enrichment_results_dir,'KEGG_Enrichment_Analysis_of_Up_and_Down_Clusters')
#     main_plot_list <- c(main_plot_list ,list(heatmaps))
...
# One_cluster=clusters
# One_cluster[]=1
# allRes <- Clusters_Enrichment_Test(enrichment_results_dir,One_cluster,KASS_Pathway2name_,KASS_Pathway2gene_,paste('KEGG_Enrichment_Analysis_of_All_Significant_Genes',SPLIT,sep="_"),"KEGG",pAdjustMethod=ENRICHMENT_PADJ_METHOD,pvalueCutoff=ENRICHMENT_PVAL_CUTOFF,gene2ko=FALSE,maxCategory = MAX_TERMS_IN_DOTPLOT)
# main_plot_list <- c(main_plot_list ,list(allRes[[2]]) )
# if (opt$Enriched_terms_overlap){
#   heatmaps = plot_shared_genes(allRes[[1]]@compareClusterResult,enrichment_results_dir,'KEGG_Enrichment_Analysis_of_All_Significant_Genes')
#   main_plot_list <- c(main_plot_list ,list(heatmaps))
...
# 
# if (((length(down_reg)>0) || (length(up_reg)>0)) && (is.na(opt$LRT))){
#   pathway_table = create_pathway_table(enrichment_results_dir,up_reg,down_reg,Annotation,KASS_Pathway2name,KASS_Pathway2gene,Pathway_show_cutoff,T,Pathway_up_color,Pathway_down_color)
# } else{
#   pathway_table = create_pathway_table(enrichment_results_dir,sig.genes,c(),Annotation,KASS_Pathway2name,KASS_Pathway2gene,Pathway_show_cutoff,T,Pathway_up_color,Pathway_down_color)
# }
...
Doing this on the complete data, including day1. To do this, I removed the day filtering in section 
Vered: In the Cherax dataset, it only outputs the current tissue
...
#store
if (exists("trinotate_main")) {
  save(analysis_name, cutoffs, trinotate, col_data, contrasts_data, norm_counts, norm_log_counts, stats_df, DE_genes, DE_genes_stats, mat2plot, corrs2, binary_patterns_stats, clusters, ORGANISM_Pathway2name_, ORGANISM_Pathway2gene_, enrichment_tables, file=r_objects_file)
} else if (exists("annot")) {
  save(analysis_name, cutoffs, annot,     col_data, contrasts_data, norm_counts, norm_log_counts, stats_df, DE_genes, DE_genes_stats, mat2plot, corrs2, binary_patterns_stats, clusters, ORGANISM_Pathway2name_, ORGANISM_Pathway2gene_, enrichment_tables, file=r_objects_file)
}
