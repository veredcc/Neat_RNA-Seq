...
count_cutoff_per_pattern_matrix = calc_pattern_1s_pass_min_counts (norm_counts_DE, patterns, replicates_structure, COUNTS_CUTOFF_HIGH)

#same, but add a condition that all zeros in the pattern must have no. counts below a cutoff
count_cutoff_per_pattern_matrix1 = calc_pattern_1s_and_0s_pass_counts_cutoffs (norm_counts_DE, patterns, replicates_structure, COUNTS_CUTOFF_HIGH, COUNTS_CUTOFF_LOW)
...
binary_pattern_results = calc_best_pattern_per_gene (corrs, CORR_CUTOFF, corr_to_patterns_file, corr_to_patterns_stats_file, count_cutoff_per_pattern_matrix, count_cutoff_per_pattern_matrix1)
...
if (exists("annot")) {
  res_df3 <- res_df %>%
    left_join(annot, by = c("gene" = colnames(annot)[1]))  # Use the first column of annot as the key
  
  if (identical(res_df$gene, res_df3$gene)) {
    res_df<-res_df3
    rm(res_df3)
    gc()
  } else {
    stop("Number and order of rows in res_df changed during the join with annot data!")
  }
}
...
res_df <- res_df %>%
    bind_cols(as.data.frame(norm_counts))

#add stats results

if (CALC_CONTRASTS & CALC_INTERACTION) {
    res_df <- res_df %>%
        left_join(stats_df_combined, by = "gene")
} else if (CALC_CONTRASTS) {
    res_df <- res_df %>%
        left_join(stats_df, by = "gene")
} else if (CALC_INTERACTION) {
    res_df <- res_df %>%
        left_join(stats_df_interaction, by = "gene")
}

#add trinotate (if exists)

if (exists("trinotate_main")) {
    res_df <- res_df %>%
        left_join(trinotate_main, by = c("gene" = "X.gene_id"))
}

# Check for duplicate gene entries
if ((res_df$gene %>% length()) != (res_df$gene %>% unique() %>% length())) {
    warning("Duplicate gene entries found in res_df!")
}
...
for (clust_method in names(gene_lists)) {
  ...
  for (clust_round in names(gene_lists[[clust_method]])) {
    ...
    for (GO_type in c('BP', 'MF', 'CC')){   #c('BP', 'MF', 'CC') or part of it
      ...
      #store enrichment results object for drawing shared genes/pathways for specific clustering rounds
      # Check if allRes is empty after the function call  #condition added on 6.1.2025
      if (length(allRes) > 0) {
        enrichment_results          [[GO_type_title]][[clust_method]][[clust_round]] = allRes[[1]]
        enrichment_results [[GO_type_title_simplify]][[clust_method]][[clust_round]] = allRes[[2]]
        enrichment_table            [[GO_type_title]][[clust_method]][[clust_round]] = allRes[[3]]
        ...
        # Handle the case where allRes is empty
        warning("No enrichment results found.") 
        # enrichment_results         [[GO_type_title]][[clust_method]][[clust_round]] <- NA
        # enrichment_results[[GO_type_title_simplify]][[clust_method]][[clust_round]] <- NA
        # enrichment_table           [[GO_type_title]][[clust_method]][[clust_round]] <- NA
        # enrichment_table  [[GO_type_title_simplify]][[clust_method]][[clust_round]] <- NA
      }
    }
    cat ("\n")
  }
...
#need to add a condition to check if there are enrichment results for the requested clustering method round (see changes on 6.1.2025 above)
  ...
  for (contrast in names(ranked_genes[[ranking_method]])) {
    ...
    if (length(res$ID)>0) {
      for (pathway_nr in 1:length(res$ID)) {
        ...
        clustering_result = gene_lists[["manual_clustering"]][["FC1.2"]]   #specify requested clustering result. Hardcoding is BAD!!
    ...
    for (contrast in names(ranked_genes[[ranking_method]])) {
      ...
      if (length(res$ID)>0) {
        for (pathway_nr in 1:length(res$ID)) {
          ...
          leading_edge_genes = res$core_enrichment[pathway_nr] %>% stringi::stri_split(regex = "/") %>% unlist
          genes_in_pathway = res@geneSets[res$ID[pathway_nr]]
          if (length(leading_edge_genes) > 1) {
            #get norm_log_counts counts
            mat2plot_le = filter_expression_matrix_by_gene_list (norm_log_counts, leading_edge_genes)
            #construct graph title
            nr_genes = length(leading_edge_genes)
            ...
            cat ("Less than 2 leading edge genes. Can't plot heatmap.")
          }
          ...
          ordered_genes = orig_genes[pheatmap_data_core_genes$tree_row$order]  #order genes by hierarchical clustering order
          #res_df2write_DE <- res_df2write[ordered_genes,] %>% select (-starts_with ("manual_cutoffs"))  #add expression and statistics data
          #the line above did not work because it expected res_df2write to have genes as row names, which was not the case.
          ordered_genes_df <- data.frame(gene = ordered_genes)
          res_df2write_DE <- ordered_genes_df %>% inner_join(res_df2write, by = "gene")
          # # Add manual cluster
          # clustering_result = man_clusters_opt1_ordered                      #specify clustering object (named vector)
          # clustering_result = gene_lists[["manual_clustering"]][["FC1.2"]]   #or choose from gene_lists
          # man_clusters_df = make_data_frame_from_clusters(clustering_result) %>% setNames(c("gene", "manual_cluster"))
          # res_df2write_DE1 = left_join(x=res_df2write_DE, y=man_clusters_df, by="gene")
          # Add manual cluster (or any clustering result of interest)
          res_df2write_DE1 = res_df2write_DE  #in case you do want to use the commented out code above
...
term_db        = "GO_BP"               #KEGG, GO_BP, GO_MF, GO_CC
ranking_method = "pval_wo_direction"  #"pval_wo_direction", "pval_with_direction", "fc"
contrast       = "G.vs.SwG"             #for method "pval_wo_direction there is also "any_contrast"

#results directory
pathway_results_dir = file.path (GSEA_results_main_dir, term_db, "specific_pathways") %>% create_dir
...
pathway_nr  = 1                               #or provide the order in the GSEA results directly
...
cutoffs = list(LINEAR_FC_CUTOFF=LINEAR_FC_CUTOFF, PADJ_CUTOFF=PADJ_CUTOFF, CORR_CUTOFF=CORR_CUTOFF, COUNTS_CUTOFF_HIGH=COUNTS_CUTOFF_HIGH, COUNTS_CUTOFF_LOW=COUNTS_CUTOFF_LOW, ENRICHMENT_PVAL_CUTOFF=ENRICHMENT_PVAL_CUTOFF, ENRICHMENT_PADJ_METHOD=ENRICHMENT_PADJ_METHOD, EFFECT1=EFFECTS[1], EFFECT2=EFFECTS[2], GROUP=GROUP, GROUP1=GROUP1)
