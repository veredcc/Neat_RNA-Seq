...
```{r preparations}
analysis_name <- sprintf("%s_%s",project_name, analysis_round)
...
partition_clustering_dir <- sprintf("%s/%s",results_dir, clustering_title)  
...
Pathway2name_file = file.path(functional_annot_dir,paste('KEGG_pathway2name','tab', sep = '.'))
Pathway2gene_file = file.path(functional_annot_dir,paste('KEGG_pathway2gene','tab', sep = '.'))
...
#per cluster
allRes  <- Clusters_Enrichment_Test(term_dir,clusters,Pathway2name,Pathway2gene,'KEGG_Enrichment_Analysis_of_Clusters', "KEGG",pAdjustMethod=ENRICHMENT_PADJ_METHOD,pvalueCutoff=ENRICHMENT_PVAL_CUTOFF,gene2ko=FALSE,maxCategory = MAX_TERMS_IN_DOTPLOT)
main_plot_list <- c(main_plot_list ,list(allRes[[2]]) )
...
#per contrast
allRes_contrasts = Contrasts_Enrichment_Test(term_dir, gene_lists$contrasts, Pathway2name, Pathway2gene, 'KEGG_Enrichment_Analysis_of_Contrasts', "KEGG", pAdjustMethod=ENRICHMENT_PADJ_METHOD, pvalueCutoff=ENRICHMENT_PVAL_CUTOFF,gene2ko=FALSE,maxCategory = MAX_TERMS_IN_DOTPLOT)
...
gseaplot2(res, geneSetID = "path:mmu04714", title = res$Description[3])  #assuming path:mmu04714 is the third in the results
...
#Plot GSEA graph of a specific pathway
gseaplot2(res1, geneSetID = 3, title = res1$Description[3]) 
...
#Plot GSEA graph of a specific pathway
gseaplot2(res2, geneSetID = 3, title = res2$Description[3]) 
...
if (exists("trinotate_main")) {
  ...
  #store
  save(analysis_name, cutoffs, trinotate, col_data, contrasts_data, norm_counts, norm_log_counts, stats_df, DE_genes, DE_genes_stats, mat2plot, corrs2, binary_patterns_stats, clusters, Pathway2name, Pathway2gene, enrichment_tables, file=r_objects_file)
} else if (exists("annot")) {
  save(analysis_name, cutoffs, annot,     col_data, contrasts_data, norm_counts, norm_log_counts, stats_df, DE_genes, DE_genes_stats, mat2plot, corrs2, binary_patterns_stats, clusters, Pathway2name, Pathway2gene, enrichment_tables, file=r_objects_file)
}
